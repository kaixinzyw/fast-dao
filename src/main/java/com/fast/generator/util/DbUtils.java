...
import com.fast.generator.bean.ColumnInfo;
import com.fast.generator.bean.BeanInfo;
...
public class DbUtils {
     ...
     */
    private final Map<String, TableInfo> beanTableInfoAll = new HashMap<>();
...
//        tableInfo.setPojoFieldsFilePath(javabasePath + tableInfo.getPojoFieldsPackPath().replace(".", separator) + separator + tableInfo.getPojoFieldsName() + ".java");
    ...
    public void setPrefix(FileCreateConfig conf, TableInfo tableInfo) {
        ...
        if (conf.getPrefix()) {
            if (StrUtil.isNotEmpty(conf.getPrefixName())) {
                ...
                int prefixIndex = tableInfo.getTableName().indexOf("_");
                if (prefixIndex < 0) {
    ...
    public static String getFieldType(String columnType, Set<String> packages) {
        ...
        if ("varchar".equals(columnType) || "nvarchar".equals(columnType) || "char".equals(columnType)
                || "text".equals(columnType) || "mediumtext".equals(columnType))
        // columnType.equals("tinytext")||columnType.equals("mediumtext")||columnType.equals("longtext")
        {
            return "String";
        } else if ("tinyblob".equals(columnType) || "blob".equals(columnType) || "mediumblob".equals(columnType)
                || "longblob".equals(columnType)) {
            return "byte[]";
        } else if ("datetime".equals(columnType) || "date".equals(columnType) || "timestamp".equals(columnType)
                || "time".equals(columnType) || "year".equals(columnType)) {
            packages.add("import java.util.Date;");
            return "Date";
        } else if ("bit".equals(columnType)) {
            return "Boolean";
        } else if ("int".equals(columnType) || "tinyint".equals(columnType)

...
模板文件生成后,会对数据库中表创建相对应的操作模板,例数据库中的demo表
--两种模板创建方式
    1.创建无初始化条件模板
...
DemoTemplate template = DemoTemplate.create();
范围条件设置
    //指定字段值范围条件,例-年龄在20-30之间 template.age().between(20, 30)
    template.字段名().between(min, max)

排序设置
    //指定字段SELECT查询时进行排序,可设置多个
...
指定字段查询设置:
    //SELECT查询时候只查询出指定字段,可设置多个
    template.字段名().showField();

过滤字段查询设置:
    //SELECT查询时候不查询指定字段,可设置多个
...
SQL条件设置:
    //会在WHERE后拼接自定义SQL语句,如果有参数需要使用#{参数名}进行占位,并在参数值MAP集合put(参数名,参数值)
    template.andSql(SQL语句,参数)或template.orSql(SQL语句,参数);
    template.orSql(SQL语句,参数)或template.orSql(SQL语句,参数);
...
OR条件设置
    //指定字段OR条件设置,例-年龄等于20或为null template.age().valEqual(20).or().isNull();
    template.字段名().or();
----
